...
export default function ChartAreaInteractive({ trend = [] }: ChartAreaInteractiveProps) {
  ...
  const filteredData = runtimeChartData.filter((item) => {
    const date = new Date(item.date)
    const latestDate = runtimeChartData[runtimeChartData.length - 1]?.date || "2024-06-30"
  ...
  return (
    <Card className="@container/card">
      <CardHeader>
        <CardTitle>Activite pipeline</CardTitle>
        <CardDescription>
