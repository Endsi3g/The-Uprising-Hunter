...
import { IconArrowLeft, IconChecklist, IconPlus, IconRocket, IconTargetArrow, IconBrandWhatsapp, IconPhone, IconMail, IconBrandLinkedin, IconCalendar } from "@tabler/icons-react"
...
type Appointment = { id: string; title: string; start_at: string; end_at: string; status: string; meeting_link?: string | null; location?: string | null }
...
export default function LeadDetailPage() {
  ...
  const { data: appointments, isLoading: appointmentsLoading, mutate: mutateAppointments } = useSWR<Appointment[]>(id ? `/api/v1/admin/leads/${id}/appointments` : null, fetcher)
    ...
    <SidebarProvider style={{ "--sidebar-width": "calc(var(--spacing) * 72)", "--header-height": "calc(var(--spacing) * 12)" } as React.CSSProperties}>
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col gap-4 p-3 pt-0 sm:p-4 sm:pt-0 lg:p-6">
          ...
          <div className="flex flex-wrap items-center gap-3"><Button variant="ghost" size="icon" onClick={() => router.push("/leads")}><IconArrowLeft className="h-5 w-5" /></Button><h1 className="text-2xl font-bold">{lead.first_name} {lead.last_name}</h1><div className="ml-auto flex flex-wrap gap-2"><Badge variant="info">{leadStatusLabel(lead.status)}</Badge><Badge>{lead.score.tier}</Badge><Badge variant="secondary">Score {Math.round(lead.total_score)}</Badge><Badge variant="info">Stage {normalizeCanonical(lead.stage_canonical)}</Badge><Badge variant="secondary">Owner {ownerDisplayName}</Badge></div></div>
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div className="space-y-4">
              <Tabs defaultValue="infos" className="space-y-4">
                <TabsList className="flex w-full flex-wrap justify-start">
                  <TabsTrigger value="infos">Infos</TabsTrigger>
                  <TabsTrigger value="interactions">Interactions timeline</TabsTrigger>
                  <TabsTrigger value="tasks">Taches</TabsTrigger>
                  <TabsTrigger value="appointments">Rendez-vous</TabsTrigger>
                  <TabsTrigger value="opportunities">Opportunites</TabsTrigger>
                ...
                <TabsContent value="interactions"><Card><CardHeader><CardTitle>Timeline interactions</CardTitle></CardHeader><CardContent className="space-y-2">{interactionsLoading ? <Skeleton className="h-24 w-full" /> : null}{(interactions || []).length === 0 ? <p className="text-sm text-muted-foreground">Aucune interaction.</p> : null}{(interactions || []).map((item) => <div key={item.id} className="rounded-lg border p-3"><div className="flex items-center justify-between"><Badge variant="info">{item.type}</Badge><span className="text-xs text-muted-foreground">{formatDateTimeFr(item.timestamp)}</span></div><p className="mt-1 text-xs text-muted-foreground">{JSON.stringify(item.details || {})}</p></div>)}</CardContent></Card></TabsContent>
                                ...
                                <TabsContent value="appointments"><Card><CardHeader className="gap-3 sm:flex-row sm:items-center sm:justify-between"><CardTitle>Rendez-vous</CardTitle><Button onClick={() => setAppointmentModalOpen(true)}><IconCalendar className="h-4 w-4" />Programmer</Button></CardHeader><CardContent className="space-y-2">{appointmentsLoading ? <Skeleton className="h-24 w-full" /> : null}{(appointments || []).length === 0 && !appointmentsLoading ? <p className="text-sm text-muted-foreground">Aucun rendez-vous programmé.</p> : null}{(appointments || []).map((item) => <div key={item.id} className="flex items-center justify-between rounded-lg border p-3"><div><p className="font-medium">{item.title}</p><div className="flex gap-2 text-xs text-muted-foreground"><span>{formatDateTimeFr(item.start_at)}</span><span>|</span><span>{item.status}</span></div>{item.meeting_link && <a href={item.meeting_link} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline">{item.meeting_link}</a>}</div><Badge variant="info">{item.status}</Badge></div>)}</CardContent></Card></TabsContent>
                                <TabsContent value="opportunities">
                <Card><CardHeader className="gap-3 sm:flex-row sm:items-center sm:justify-between"><CardTitle>Opportunites</CardTitle><div className="grid w-full gap-2 sm:w-auto sm:grid-cols-[1fr_110px_150px_auto]"><Input value={oppName} onChange={(e) => setOppName(e.target.value)} placeholder="Nom" /><Input type="number" min={0} value={oppAmount} onChange={(e) => setOppAmount(e.target.value)} placeholder="Montant" /><select title="Opportunity stage" className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={oppStage} onChange={(e) => setOppStage(e.target.value)}>{STAGE_OPTIONS.map((item) => <option key={item} value={item}>{item}</option>)}</select><Button onClick={() => void createOpportunity()} disabled={busy}><IconPlus className="h-4 w-4" />Ajouter</Button></div></CardHeader><CardContent className="space-y-2">{opportunitiesLoading ? <Skeleton className="h-24 w-full" /> : null}{(opportunities || []).map((item) => <div key={item.id} className="rounded-lg border p-3"><div className="flex items-center justify-between"><p className="font-medium">{item.name}</p><div className="flex gap-2"><Badge variant="info">{item.stage}</Badge><Badge>{item.status}</Badge></div></div><p className="text-xs text-muted-foreground">Montant: {formatCurrencyFr(item.amount)} | Probabilite: {item.probability}% | Maj: {formatDateTimeFr(item.updated_at || null)}</p></div>)}</CardContent></Card></TabsContent>
      ...
      <BookAppointmentModal
        ...
        leadId={id}
        leadName={`${lead.first_name} ${lead.last_name}`}
        onSuccess={() => void mutateAppointments()}
      />
    </SidebarProvider>
  )
}
