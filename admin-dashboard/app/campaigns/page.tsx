...
export default function CampaignsPage() {
  ...
  async function createCampaign(event: React.FormEvent<HTMLFormElement>) {
    ...
    const strategy = parseJsonObject(campaignStrategy, "channel_strategy")
    if (strategy === null) return
    const filter = parseJsonObject(campaignFilter, "enrollment_filter")
    if (filter === null) return
    setCampaignBusy(true)
    try {
      await requestApi("/api/v1/admin/campaigns", {
        method: "POST",
  ...
  async function createSequence(event: React.FormEvent<HTMLFormElement>) {
    ...
    const steps = parseJsonArray(sequenceSteps, "steps")
      ...
      <div className="flex flex-1 flex-col gap-4">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="campaigns" className="space-y-4">
              <Card>
                <CardHeader>
                  <CardTitle>Creation de campagne</CardTitle>
                </CardHeader>
                <CardContent>
                  <form className="grid gap-3 md:grid-cols-2 xl:grid-cols-4" onSubmit={createCampaign}>
                    ...
                    <div className="space-y-1 md:col-span-2">
                      <Label htmlFor="campaign-strategy">Channel strategy (JSON)</Label>
                      <textarea
                    ...
                    <div className="space-y-1 md:col-span-2">
                      <Label htmlFor="campaign-filter">Enrollment filter (JSON)</Label>
                      <textarea
                        id="campaign-filter"
                    ...
                    <div className="md:col-span-4">
                      <Button type="submit" disabled={campaignBusy}>
                        {campaignBusy ? "Creation..." : "Creer la campagne"}
                      </Button>
                    </div>
                  </form>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <form className="grid gap-3 md:grid-cols-2" onSubmit={createSequence}>
                    ...
                    <div className="space-y-1 md:col-span-2">
                      <Label htmlFor="sequence-steps">Steps (JSON array)</Label>
                      <textarea
                        id="sequence-steps"
                        value={sequenceSteps}
                      ...
                      sequences.map((sequence) => (
                        <button
                          ...
                          className={`w-full rounded-lg border p-2 text-left ${
                            selectedSequenceId === sequence.id ? "border-primary bg-muted/40" : ""
                          }`}
