...
class BomboraIntentClient(IntentProviderClient):
    provider_name = "bombora"

    def __init__(self, api_key: str, base_url: Optional[str] = None, timeout: int = 10):
        self.api_key = api_key
        self.base_url = (base_url or "https://api.bombora.com").rstrip("/")
    ...
    def fetch_company_intent(
        ...
        try:
            payload = request_json(
                self.session,
                "GET",
                f"{self.base_url}/intent/company",
                config=self.request_config,
                params=params,
            )
        except requests.RequestException as exc:
            logger.warning(
                "Bombora intent request failed.",
                extra={
